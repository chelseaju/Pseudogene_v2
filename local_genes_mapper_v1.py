""" locla_genes_mapper_v1.py
Usage: python local_genes_mapper_v1.py -d directory  -r reference gene name
Input: -d input/output directory -r reference gene name
...
Function: 1. intersect a list of reads to ensembl gene database
          2. match unknown region with reference gene name 
...
"""
	Function : build the reference for unknown region
"""
def build_unknown_ref(unknown_ref):
	fh = open(unknown_ref, "r")
	fh.readline()
	colnames = fh.readline().rstrip().split()
	
	for name in colnames:
		if(re.match('^unknown', name)):
			(u, chr, start, end) = name.split("_")
			UNKNOWN_REF.append((chr, start, end, name))
	
	fh.close()
		
"""
	Function: find the closest global unknown region
"""
def find_match_unknown(loci):

	(u, loci_chr, loci_start, loci_end) = loci.split("_")
	for (u_chr, u_start, u_end, u_name) in UNKNOWN_REF:
		if(loci_chr == u_chr):
			# check overlap
			if((int(loci_start) >= int(u_start) and int(loci_start) <= int(u_end)) or (int(u_start) >= int(loci_start) and int(u_start) <= int(loci_end))):
				return u_name
	return loci
...
def export_temp_unknown(unknown_list, unknown_file):
    ...
    echo("Write to Temporary File : %s" %(unknown_file))
...
#	unknown = map_reads_to_gene(output_temp, ENSEMBL_GENE)
	...
	# output distribution
	compute_distribution(output_file)


if __name__ == "__main__":   
   
	parser = argparse.ArgumentParser(prog='genes_mapper_v1.py')
	parser.add_argument("-d", "--directory", dest="dir", type=str, help="directory of input and output files", required = True)
	parser.add_argument("-r", "--reference", dest="ref", type=str, help="reference gene name", required = True)	
	main(parser)
